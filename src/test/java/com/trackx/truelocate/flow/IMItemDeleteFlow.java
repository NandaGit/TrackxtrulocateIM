...
public class IMItemDeleteFlow extends GeneralActions {
	WebDriver driver;
	Truelocatelogin truelocatelogin;
	IMItemElements itemelements ;
	...
	public void setUp() throws IOException {
		...
		itemelements = PageFactory.initElements(driver, IMItemElements.class);
	...
	public void itemDeleteFlow(String sValue)throws Exception {
		try {
			itemelements.menuClick();
			Thread.sleep(1000);
			ReusableActions.takeSnapshot(driver, className);
			...
			if (alertMessage.equalsIgnoreCase(constants.delete_item_successmsg)) {
				TestNGResults.put("20", new Object[] { "Item delete screen",
						"Item delete successfully", "Pass" });	
				Assert.assertEquals(alertMessage, constants.delete_item_successmsg);
			} else {
				TestNGResults.put("20", new Object[] { "Item delete screen",
						"Item not deleted", "Fail" });
				Assert.assertEquals(alertMessage, constants.delete_item_successmsg);
			}
		}
		catch (Exception e) {
	...
	public static Object[][] createData() {
		return GeneralActions.getData("DeleteItem");
	}
